...
export function TabsProvider({ children }: { children: ReactNode }) {
  ...
  const initializeTabs = useCallback(
    async (projectPath: string): Promise<InitializeTabsResult | null> => {
      try {
  ...
  const deleteTabById = useCallback(
    async (projectPath: string, tabId: string): Promise<boolean> => {
  ...
  const duplicateTabById = useCallback(
    async (projectPath: string, tabId: string): Promise<TabMetadata | null> => {
      try {
  ...
  const reorderTabsById = useCallback(
    async (projectPath: string, tabIds: string[]): Promise<boolean> => {
  ...
  const markTabDirty = useCallback((tabId: string) => {
    setTabs((prev) =>
      prev.map((t) => (t.id === tabId ? { ...t, hasUnsavedChanges: true } : t)),
    );
  }, []);
